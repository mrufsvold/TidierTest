...
# ╔═╡ 1e14b980-0103-47dc-ba2a-06f12f98d279
using CSV

# ╔═╡ e85444da-bae3-428d-a5ae-d7578e886101
...
Here we'll load the veteran geography file and the NPPES datasets
"""

# ╔═╡ ea07d294-e58d-4b74-822a-e8ca524e0e59
parent_dir = joinpath(splitpath(pwd())[begin:end-1])

# ╔═╡ 1e05862c-056d-49e0-a75f-264a86c8d64c
veteran_geo_file = joinpath(parent_dir, "ACSST5Y2022.S2101-Data.csv")

# ╔═╡ 8ee4a67f-afe3-4569-ba66-6523088b894f
nppes_file = joinpath(parent_dir, "nppes.csv")

# ╔═╡ 81528fcc-e3d8-4788-be2f-858a24a45bfe
md"""
## Veteran Geo
This file contains zip code level data about various demographic aspects of the veteran population across the United States.

For this analysis, we will need to:
1. Clean up the zip code by splitting it into just the 5 digit code.
2. Calculate the percent of the population that is a veteran in each zip code
"""

# ╔═╡ 1ce41de3-0b41-4d0e-b55a-0a6bb65311d1
md"""
## NPPES
The NPPES dataset is a public source of information about all registered National Provider Identifiers (NPIs) with CMS. 

For NPPES, we need to:
1) Check that all NPIs are valid. [See the CMS Spec](https://www.cms.gov/Regulations-and-Guidance/Administrative-Simplification/NationalProvIdentStand/Downloads/NPIcheckdigit.pdf) for details.
2) Flag the each npi if it has a taxonomy code that matches one of our groups of interest
3) Set the zip code to 5 digits
"""

# ╔═╡ c95b89d1-808e-40c8-8d85-dbffb4d4945d
md"""
Every NPI should pass the Luhn's check. Here, we run that against the NPIs we are using.
"""
...
md"""
# Functions
Below you will find the functions used in this notebook defined and documented.
"""

# ╔═╡ 9457b87a-4278-4d44-97df-dc8628272568
"""
	average_tenure(tenure_col, specialty_flag)
Consumes a `Vector{Dates.Day}` and a `Vector{Bool}` and calculates the average number of days all the specialists have been registered with an NPI.aq1
"""
function average_tenure(tenure_col, specialty_flag)
	specialist_tenure = view(tenure_col, specialty_flag)
	total_specialists = length(specialist_tenure)
	# if we don't have any specialists, then our average is 0
	total_specialists == 0 && return Dates.Day(0)
	total_tenure_days = sum(specialist_tenure; init=Dates.Day(0))

	# unicode `÷` is an integer divide
	average_tenure = Dates.value(total_tenure_days) ÷ total_specialists
	return Dates.Day(average_tenure)
end

# ╔═╡ a96b8513-8183-4edb-9c12-3889ab4c6a3c
"""
	divide_fill_zero(x,y)
Divide x and y into a float, but if y is zero (so result is NaN), returns `0.0` instead.
"""
function divide_fill_zero(x,y)
	if y == 0
		return 0.0
	end
	return x/y
...
# ╔═╡ b4d70bfa-7b1e-492e-a626-94daa376d194
veteran_geo_df = @chain veteran_geo_file begin 
	CSV.read(
		DataFrame;
		header=2, 
		normalizenames=true, 
		# This data set indicates small cell suppression with "(X)"
		missingstring="(X)",
		select=[
			"Geographic_Area_Name", 
			"Estimate!!Veterans!!Civilian_population_18_years_and_over",
			"Estimate!!Total!!Civilian_population_18_years_and_over"
		]
	)
	@mutate(
		zip_code = String7(getindex(split(Geographic_Area_Name), 2)),
		percent_vet_pop = divide_fill_zero(
			Estimate!!Veterans!!Civilian_population_18_years_and_over, 
			Estimate!!Total!!Civilian_population_18_years_and_over),
		total_pop = Estimate!!Total!!Civilian_population_18_years_and_over
	)
	@select(zip_code, total_pop, percent_vet_pop)
end

# ╔═╡ 4312bd4c-bafc-4527-8232-3196e508fc9d
"""
	flag_specialty(taxonomy_columns, taxonomy_pattern)

Returns a Vector{Bool} flagging rows of `taxonomy_columns` where there is a matching taxonomy code
"""
function flag_specialty(taxonomy_columns, taxonomy_pattern)
	function match_func(v)
		if v isa Missing
			return false
		end
		!(match(taxonomy_pattern, v) isa Nothing)
	end
	broadcast_or = (c1, c2) -> .|(c1,c2)
	mapreduce(
		col -> map(match_func, col),
		broadcast_or,
		eachcol(taxonomy_columns);
		init = falses(size(taxonomy_columns)[1])
	)
end

# ╔═╡ 6f348e4b-dc74-4efb-b7f8-dc6580b5253c
"""
	nppes_column_selector(_,col)

Returns a boolean if the NPPES column is desired for this analysis.
"""
function nppes_column_selector(_,col)
	s_col = string(col)
	desired_columns = Set((
		"NPI",
	...
	if s_col in desired_columns
		return true
	end
	tax_match = match(r"Healthcare_Provider_Taxonomy_Code_\d+", s_col)
	return !(tax_match isa Nothing)
end

# ╔═╡ cfbe570e-0c13-4467-84ca-910b1e4f8395
nppes_df = @chain nppes_file begin
	CSV.read(
		DataFrame;
		normalizenames=true, 
		select=nppes_column_selector,
	...
	@mutate(
		valid_state = length.(state) .== 2,
		is_active = NPI_Deactivation_Date === missing || NPI_Reactivation_Date !== missing
	)
	@filter(valid_state, is_active)
	@mutate(
		tenure = Dates.today() - Provider_Enumeration_Date,
		zip_code = String7.(getindex.(lpad.(string.(zip_code), 5), Ref(1:5))),
	)
end

# ╔═╡ 3848608a-4c10-47a1-8a95-4aaca61c37d7
begin
	# Just getting the subset of columns that contain taxonomy codes
	tax_columns = @select(nppes_df, contains("Tax"))
	# these are the regex patterns for each category of taxonomy code we want
	speciality_patterns = Dict(
		:counseling_psychology => r"(^101Y.*)|(^103T.*)",
		:physical_therapy => r"^2251.*",
		:oncology => r"207RX0202X"
	)
	# adding the specialty flags
	for (specialty_name, pattern) in pairs(speciality_patterns)
		nppes_df[!,specialty_name] = flag_specialty(tax_columns,pattern)
	end
end

# ╔═╡ 966c8105-dd1e-4ef8-97e1-5ddc09ccfa67
enriched_nppes = @chain nppes_df begin
	@select(NPI,state,zip_code, tenure,physical_therapy, counseling_psychology, oncology)
	@inner_join(veteran_geo_df)
end

# ╔═╡ b31b132f-a1bb-4537-b2da-d0ef15a29c81
zip_code_summaries = @chain enriched_nppes begin
	@group_by(zip_code)
	@summarize(
		counseling_psychology_count = sum(counseling_psychology),
		oncology_count = sum(oncology),
		physical_therapy_count = sum(physical_therapy),
		# Need to use `~` to avoid autovectorization of a function where I actually want the whole column passed
		counseling_psychology_tenure = ~average_tenure(tenure,counseling_psychology),
...
# ╔═╡ a4070ccf-3816-4ce8-8e23-36993dec8204
"""
	add_digits_less_than_20(i)

For numbers less than 20, return the sum of the digits.
"""
function add_digits_less_than_20(i)
	if i < 10
		return i
	end
	return i % 10 + 1
...
# ╔═╡ 1f0fb648-a6e1-469d-b31d-08ce4708914f
"""
	get_nth_digit(d,n)
returns the digit in the `n`s place of `d`. For example:
```julia
get_nth_digit(1234, 2) # return 3
```
"""
function get_nth_digit(d,n)
	mod_to_n = d % 10^n
	remaining_sig_figs = d % 10^(n-1)
	res = (mod_to_n - remaining_sig_figs) / (10^(n-1))
	return convert(Int,res)
end

# ╔═╡ 3d3f2bf4-81c5-477a-8619-91595eb39e6f
"""
    validate_npi(npi::Int)::Bool

Runs the Luhn Check to validate an NPI. [See the CMS Spec](https://www.cms.gov/Regulations-and-Guidance/Administrative-Simplification/NationalProvIdentStand/Downloads/NPIcheckdigit.pdf) for details.
"""
function validate_npi(npi::Int)
	# the "ones place" of an NPI is the check digit. 
	# So we separate it from the rest of the number
	expected_check_digit = npi % 10
	npi_id_base::Int64 = (npi - expected_check_digit) ÷ 10
	# the spec pads all npis with this prefix
	standard_prefix = 80840_000_000_000
	npi_id = npi_id_base + standard_prefix

	# Here, we're breaking up the NPI into a list of digits
	digits = Iterators.map(n -> get_nth_digit(npi_id, n), 1:14)
	total = sum(
		(i_v) -> begin
			isodd(i_v[1]) ? 
			# double every other value and then sum their digits if it is greater than 9
			add_digits_less_than_20(i_v[2] * 2) : 
			i_v[2]
		end,
		Iterators.enumerate(digits)
	)
	# the check digit is the total subtracted from the next highest multiple of 10
	actual_check_digit = -(total % 10 - 10) % 10
	actual_check_digit == expected_check_digit
end

# ╔═╡ 5849f16b-0617-406f-9edf-4e3739c715f6
@assert all(map(validate_npi, nppes_df.NPI)) "Some NPIs do not pass validation"
